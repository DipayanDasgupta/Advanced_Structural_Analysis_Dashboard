...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ... (all other existing DOM references are fine) ...
    ...
    // --- Theme Colors (fetched from CSS variables for JS usage) ---
    const themeColors = {
        primary: getComputedStyle(document.documentElement).getPropertyValue('--primary-color').trim() || '#007bff',
        text: getComputedStyle(document.documentElement).getPropertyValue('--text-color').trim() || '#343a40',
        border: getComputedStyle(document.documentElement).getPropertyValue('--border-color').trim() || '#e0e6ed',
        cardBackground: getComputedStyle(document.documentElement).getPropertyValue('--card-background-color').trim() || '#ffffff',
        ...
        mutedText: getComputedStyle(document.documentElement).getPropertyValue('--muted-text-color').trim() || '#6c757d',
        canvasSupportColor: '#2c3e50', // A dark neutral for supports
        canvasLoadColor: '#e74c3c',    // A distinct red for loads
        canvasUdlColor: 'rgba(39, 174, 96, 1)', // Green for UDL
    };
    ...
    function updateValueDisplay(inputElem, displayElem, unit = '', multiplier = 1, fixed = 2) {
        if (inputElem && displayElem) {
            ...
            if (isNaN(value)) value = 0; // Handle NaN if input is empty or invalid
    ...
    effLengthFactorKxInput.addEventListener('input', () => updateValueDisplay(effLengthFactorKxInput, effLengthFactorKxValueDisplay, '', 1, 2)); // K factors up to 2 decimal places
    ...
    if(columnEndConditionSelect) {
        columnEndConditionSelect.addEventListener('change', function() {
            ...
            effLengthFactorKxContainer.style.opacity = isCustom ? '1' : '0.6';
            effLengthFactorKyContainer.style.opacity = isCustom ? '1' : '0.6';

            if (!isCustom) {
                const [kx, ky] = selectedValue.split(',').map(v => parseFloat(v));
                effLengthFactorKxInput.value = kx.toFixed(2); // Ensure consistency
    ...
    function toggleElementInputs() {
        ...
        beamPlotsDiv.style.display = isBeam ? 'block' : 'none'; // HTML structure wraps plots in their own card-like divs now
    ...
    function filterBeamLoadTypes() {
        ...
        if (!currentSelectedStillVisible && firstVisibleOption) {
            beamLoadTypeSelect.value = firstVisibleOption.value;
        } else if (!currentSelectedStillVisible && !firstVisibleOption) {
            beamLoadTypeSelect.value = ""; // Should not happen if options are well-defined
        }
    ...
    function toggleBeamLoadInputs() {
        ...
        pointLoadPositionContainer.style.display = 'none'; // Default to hidden
        if(beamUdlInputsDiv) beamUdlInputsDiv.style.display = 'none';

        if (loadType === 'pointLoad') {
            beamPointLoadInputsDiv.style.display = 'block';
            pointLoadPositionContainer.style.display = 'flex'; // This one needs flex
        } else if (loadType === 'pointLoadEnd') {
            beamPointLoadInputsDiv.style.display = 'block';
            // pointLoadPositionContainer remains hidden
    ...
    async function performCalculation() {
        ...
        if (payload.sectionType === 'rectangular') {
            payload.sectionParams = [parseFloat(rectWidthInput.value), parseFloat(rectHeightInput.value)];
        } else if (payload.sectionType === 'circular') {
            payload.sectionParams = [parseFloat(circDiameterInput.value)];
        }
        ...
        if (payload.elementType === 'beam') {
            ...
            if (payload.beamLoadType === 'pointLoad' || payload.beamLoadType === 'pointLoadEnd') {
                ...
                if (payload.beamLoadType === 'pointLoad') {
                     payload.pointLoadPositionRatio = parseFloat(pointLoadPositionRatioInput.value);
                }
            } else if (payload.beamLoadType === 'udl') {
                payload.udlValue = parseFloat(udlValueInput.value);
            }
    ...
    function clearResults(clearInputs = false) { // Added option to not clear inputs
        summaryResultsDiv.innerHTML = '';
        failureChecksListDiv.innerHTML = '';
        jsonOutputPre.textContent = '';
        if (sfdPlotDiv.data) Plotly.purge(sfdPlotDiv);
        if (bmdPlotDiv.data) Plotly.purge(bmdPlotDiv);
        if (deflectionPlotDiv.data) Plotly.purge(deflectionPlotDiv);
        ...
        crossSectionStressInfoDiv.innerHTML = '';

        if (clearInputs) {
            // Optionally clear or reset input fields here if needed
        }
    }
...
function displayResults(data, inputPayload) {
    clearResults(false); // Don't clear inputs when displaying new results
    ...
    function toTitleCase(str) {
        ...
        return str.replace(/_/g, ' ') // First, replace underscores with spaces
                  .toLowerCase() // Convert everything to lower case first
    ...
    // Corrected line for material name:
    summaryHtml += `<p><strong>Material:</strong> ${toTitleCase(element_info.material.name)}</p>`;
    summaryHtml += `<p><strong>Cross-section:</strong> ${element_info.cross_section.type} (Area: ${(element_info.cross_section.area_m2 * 1e6).toFixed(1)} mm²)</p>`;
    ...
    if (inputPayload.elementType === 'beam') {
        ...
        const x_sfd = results.sfd_points.map(p => p.x);
        const v_sfd = results.sfd_points.map(p => p.v / 1000); // kN
        const x_bmd = results.bmd_points.map(p => p.x);
        const m_bmd = results.bmd_points.map(p => p.m / 1000); // kNm
        const x_defl = results.deflection_points.map(p => p.x);
        const d_defl = results.deflection_points.map(p => p.d * 1000); // mm
    ...
    if (results.failure_checks && Object.keys(results.failure_checks).length > 0) {
        for (const key in results.failure_checks) {
            ...
            const checkTitle = key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()); // Title Case

            failureHtml += `<li><strong>${checkTitle}:</strong>
                <span class="status ${statusClass}">${check.status.toUpperCase()}</span>
                <span class="details">`;
            
            if (typeof check.ratio === 'number') failureHtml += `Ratio: ${check.ratio.toFixed(3)}`;
            else failureHtml += `Ratio: ${check.ratio}`;
        ...
        failureHtml += `<li>No failure checks performed or available.</li>`;
    }
    failureHtml += '</ul>';
...
// ... (rest of the script remains the same) ...
    function plotDiagram(divElement, xData, yData, xAxisTitle, yAxisTitle, plotTitle, invertY = false) {
        const trace = {
            ...
            type: 'scatter',
            mode: 'lines+markers',
            marker: { size: 5, color: themeColors.primaryHoverColor || '#0056b3' },
            line: { color: themeColors.primary || '#007bff', width: 2.5 }
        };
        const layout = {
            title: { 
                text: plotTitle, 
                font: { family: 'var(--font-family)', size: 17, color: themeColors.text },
                x: 0.05, xanchor: 'left', // Align title left
                y: 0.95, yanchor: 'top'
            },
            xaxis: { 
                title: { text: xAxisTitle, font: { family: 'var(--font-family)', size: 13, color: themeColors.mutedText } }, 
                zeroline: true, zerolinewidth:1.5, zerolinecolor: '#bdbdbd', 
                gridcolor: themeColors.border, 
                tickfont: { family: 'var(--font-family)', color: themeColors.text }
            },
            yaxis: { 
                title: { text: yAxisTitle, font: { family: 'var(--font-family)', size: 13, color: themeColors.mutedText } }, 
                zeroline: true, zerolinewidth:1.5, zerolinecolor: '#bdbdbd', 
                autorange: invertY ? 'reversed' : true, 
                gridcolor: themeColors.border, 
                tickfont: { family: 'var(--font-family)', color: themeColors.text }
            },
            margin: { l: 70, r: 30, t: 60, b: 60 }, // Increased top margin for title
            paper_bgcolor: themeColors.cardBackground,
            plot_bgcolor: themeColors.cardBackground,
            font: { family: 'var(--font-family)', color: themeColors.text }
        };
        Plotly.newPlot(divElement, [trace], layout, {responsive: true, displaylogo: false});
    }

    // --- Canvas Drawing Functions (minor style tweaks) ---
    function drawBeamDiagram(element_info, results, inputs) {
        ...
        const y_beam_centerline = elementDiagramCanvas.height * 0.4; // Shifted up a bit
        const y_deflected_beam_center = elementDiagramCanvas.height * 0.75; // Shifted down for deflection

        let section_depth_m = 0.1;
        if (element_info.cross_section.cy_top_m !== undefined && element_info.cross_section.cy_bottom_m !== undefined) {
            section_depth_m = element_info.cross_section.cy_top_m + element_info.cross_section.cy_bottom_m;
        } else if (element_info.cross_section.type === "Circular" && element_info.cross_section.cx_right_m !== undefined) {
             section_depth_m = 2 * element_info.cross_section.cx_right_m;
        }
        const beam_thickness_px = Math.max(10, Math.min(30, section_depth_m * scale * 0.35 + 6)); // Slightly thicker
        ...
        if (max_abs_moment_val > 0 && results.bmd_points) {
            for (let i = 0; i < results.bmd_points.length - 1; i++) {
                ...
                const color = getColorForValue(Math.abs(segment_moment_avg), 0, max_abs_moment_val); // Use existing color logic
        ...
        elementCtx.strokeStyle = themeColors.canvasSupportColor || '#34495e';
        elementCtx.lineWidth = 2; // Thicker outline
        elementCtx.strokeRect(margin_x, y_beam_centerline - beam_thickness_px / 2, L_px, beam_thickness_px);
        elementCtx.lineWidth = 1;

        const support_size = 18; // Slightly larger supports
        if (inputs.beamSupportType === 'simplySupported') {
            drawTriangleSupport(elementCtx, margin_x, y_beam_centerline + beam_thickness_px / 2, support_size, themeColors.canvasSupportColor);
            drawRollerSupport(elementCtx, margin_x + L_px, y_beam_centerline + beam_thickness_px / 2, support_size, themeColors.canvasSupportColor);
        } else if (inputs.beamSupportType === 'cantilever') {
            elementCtx.fillStyle = themeColors.canvasSupportColor;
            elementCtx.fillRect(margin_x - support_size, y_beam_centerline - beam_thickness_px*1.3, support_size, beam_thickness_px * 2.6);
            elementCtx.strokeStyle = themeColors.canvasSupportColor;
            elementCtx.strokeRect(margin_x - support_size, y_beam_centerline - beam_thickness_px*1.3, support_size, beam_thickness_px * 2.6);
            for (let i = 0; i < 6; i++) { // More hatching lines
        ...
        const load_arrow_top_y = y_beam_centerline - beam_thickness_px / 2 - 30; // More space for load arrow
        const load_arrow_bottom_y = y_beam_centerline - beam_thickness_px / 2 - 3;
        elementCtx.textAlign = 'center';
        elementCtx.font = "13px " + themeColors.fontFamily; // Consistent font
        ...
        if (inputs.beamLoadType.includes('pointLoad')) {
            ...
            drawArrow(elementCtx, load_x_px, load_arrow_top_y, load_x_px, load_arrow_bottom_y, themeColors.canvasLoadColor, 12); // Larger arrowhead
            elementCtx.fillStyle = themeColors.canvasLoadColor;
            elementCtx.fillText(`${inputs.pointLoad || 0} kN`, load_x_px, load_arrow_top_y - 8);
        } else if (inputs.beamLoadType === 'udl') {
            const numArrows = Math.min(12, Math.max(5, Math.floor(L_px / 50))); // Adjusted arrow density
            const udlRectTopY = load_arrow_top_y + 5; // UDL rectangle slightly lower
            const udlRectBottomY = load_arrow_bottom_y;

            elementCtx.fillStyle = themeColors.canvasUdlColor.replace('1)', '0.15)'); // Lighter fill for UDL area
            elementCtx.fillRect(margin_x, udlRectTopY, L_px, udlRectBottomY - udlRectTopY);
            elementCtx.strokeStyle = themeColors.canvasUdlColor.replace('1)', '0.6)'); // Softer border for UDL area
            ...
            for (let i = 1; i <= numArrows; i++) {
                ...
                drawArrow(elementCtx, arrowX, udlRectTopY, arrowX, udlRectBottomY, themeColors.canvasUdlColor, 7); // Smaller arrows for UDL
            }
            elementCtx.fillStyle = themeColors.canvasUdlColor;
        ...
        if (deflection_points && deflection_points.length > 0) {
            ...
            let exaggeration_factor = max_abs_deflection_m > 0 ? (elementDiagramCanvas.height * 0.20) / max_abs_deflection_m : 0; // More exaggeration
            ...
            for (let i = 1; i < deflection_points.length; i++) {
                elementCtx.lineTo(margin_x + (deflection_points[i].x || 0) * scale, y_deflected_beam_center + (deflection_points[i].d || 0) * exaggeration_factor);
            }
            elementCtx.strokeStyle = themeColors.primary + 'AA'; // Primary color with alpha
            elementCtx.lineWidth = 2.5; // Thicker deflected line
            elementCtx.stroke();
            elementCtx.lineWidth = 1;
        }
         // Add Length Dimension
        elementCtx.strokeStyle = themeColors.mutedText;
        elementCtx.lineWidth = 0.75;
        const dimY = y_beam_centerline + beam_thickness_px / 2 + support_size + 25;
        elementCtx.beginPath();
        elementCtx.moveTo(margin_x, dimY - 5); elementCtx.lineTo(margin_x, dimY + 5); // Left tick
        elementCtx.moveTo(margin_x + L_px, dimY - 5); elementCtx.lineTo(margin_x + L_px, dimY + 5); // Right tick
        elementCtx.moveTo(margin_x, dimY); elementCtx.lineTo(margin_x + L_px, dimY); // Dimension line
        elementCtx.stroke();
        elementCtx.fillStyle = themeColors.mutedText;
        elementCtx.textAlign = 'center';
    ...
    function drawColumnDiagram(element_info, results, inputs) {
        elementCtx.clearRect(0, 0, elementDiagramCanvas.width, elementDiagramCanvas.height);
        const H_px = elementDiagramCanvas.height * 0.75; // Use more height
        const H_m = element_info.length_m;
        ...
        if (element_info.cross_section.cx_left_m !== undefined && element_info.cross_section.cx_right_m !== undefined) {
            section_width_m = element_info.cross_section.cx_left_m + element_info.cross_section.cx_right_m;
        } else if (element_info.cross_section.type === "Circular" && element_info.cross_section.cx_right_m !== undefined) {
             section_width_m = 2 * element_info.cross_section.cx_right_m;
        }
        const col_width_px = Math.max(12, Math.min(45, section_width_m * scale_y * 0.6 + 8)); // Thicker column viz
        ...
        if (yield_strength_Pa > 0 && yield_strength_Pa !== Infinity) {
            const color = getColorForStress(-axial_stress_Pa, yield_strength_Pa); // Assuming +ve stress from backend is compressive
            elementCtx.fillStyle = color;
            elementCtx.fillRect(x_col_center - col_width_px / 2, margin_y_top, col_width_px, H_px);
        } else {
            elementCtx.fillStyle = '#bdc3c7'; // Default grey
            elementCtx.fillRect(x_col_center - col_width_px / 2, margin_y_top, col_width_px, H_px);
        }
        ...
        const support_size = 18; // Consistent support size
        drawTriangleSupport(elementCtx, x_col_center, margin_y_top + H_px + support_size / 2, support_size, themeColors.canvasSupportColor, 'bottom');
        drawTriangleSupport(elementCtx, x_col_center, margin_y_top - support_size / 2, support_size, themeColors.canvasSupportColor, 'top');

        drawArrow(elementCtx, x_col_center, margin_y_top - 30, x_col_center, margin_y_top -3, themeColors.canvasLoadColor, 12);
        elementCtx.fillStyle = themeColors.canvasLoadColor;
        elementCtx.textAlign = 'center';
        ...
        if(results.failure_checks && results.failure_checks.euler_buckling) {
            const buckling_check = results.failure_checks.euler_buckling;
            if (buckling_check.ratio > 0.3 && P_crit > 0) { // Show if ratio is somewhat significant
                buckling_mode_visible = true;
            }
        }
        ...
        if (buckling_mode_visible) {
            ...
            elementCtx.beginPath();
            elementCtx.moveTo(x_col_center, margin_y_top);
            
            // Use effective length factor to influence shape (very simplified)
            // This is highly approximate for visualization only.
            let K_factor_for_shape_approx = Math.min(element_info.Kx, element_info.Ky); // Buckle in weakest K direction
            if (K_factor_for_shape_approx <= 0.6) { // Fixed-Fixed like
                for (let y_rel = 0; y_rel <= H_px; y_rel++) {
                    const y_norm = y_rel / H_px;
                    const buckled_offset = buckling_exaggeration_px * 0.5 * (1 - Math.cos(2 * Math.PI * y_norm));
                    elementCtx.lineTo(x_col_center + buckled_offset, margin_y_top + y_rel);
                }
            } else if (K_factor_for_shape_approx >= 1.8) { // Fixed-Free like
                 for (let y_rel = 0; y_rel <= H_px; y_rel++) {
                    const y_norm = y_rel / H_px;
                    const buckled_offset = buckling_exaggeration_px * (1 - Math.cos( (Math.PI / 2) * y_norm) );
                    elementCtx.lineTo(x_col_center + buckled_offset, margin_y_top + y_rel);
                }
            } else { // Pinned-Pinned (default) or Fixed-Pinned
                for (let y_rel = 0; y_rel <= H_px; y_rel++) {
                    const y_norm = y_rel / H_px;
                    const buckled_offset = buckling_exaggeration_px * Math.sin(Math.PI * y_norm);
                    elementCtx.lineTo(x_col_center + buckled_offset, margin_y_top + y_rel);
                }
            }
            elementCtx.strokeStyle = themeColors.danger + 'AA'; // Danger color with alpha
            elementCtx.lineWidth = 2.5; // Thicker buckled line
            elementCtx.stroke();
            elementCtx.lineWidth = 1;
        }
        ...
        elementCtx.beginPath();
        elementCtx.moveTo(dimX - 5, margin_y_top); elementCtx.lineTo(dimX + 5, margin_y_top); // Top tick
        elementCtx.moveTo(dimX - 5, margin_y_top + H_px); elementCtx.lineTo(dimX + 5, margin_y_top + H_px); // Bottom tick
        elementCtx.moveTo(dimX, margin_y_top); elementCtx.lineTo(dimX, margin_y_top + H_px); // Dimension line
        elementCtx.stroke();
        elementCtx.fillStyle = themeColors.mutedText;
        elementCtx.textAlign = 'center';
        elementCtx.font = "12px " + themeColors.fontFamily;
        ...
        elementCtx.save();
        elementCtx.translate(dimX + 8, margin_y_top + H_px / 2);
        elementCtx.rotate(-Math.PI / 2);
        elementCtx.fillText(`H = ${H_m.toFixed(1)} m`, 0, 0);
        elementCtx.restore();
        elementCtx.textAlign = 'start';
    }
    ...
    function drawTriangleSupport(ctx, x, y, size, color, orientation = 'bottom') {
        ...
        if (orientation === 'bottom') {
            ctx.moveTo(x - size, y); ctx.lineTo(x + size, y); ctx.lineTo(x, y - size);
        } else { // Top
            ctx.moveTo(x - size, y); ctx.lineTo(x + size, y); ctx.lineTo(x, y + size);
        }
        ctx.closePath(); ctx.fillStyle = color; ctx.fill();
        // Add ground hatching for bottom support
        if (orientation === 'bottom') {
            ctx.strokeStyle = color; ctx.lineWidth = 0.8;
            for (let i = 0; i < 5; i++) {
                ctx.beginPath();
                const startX = x - size + (i * (2 * size / 4));
                ctx.moveTo(startX, y);
                ctx.lineTo(startX - size/3, y + size/3);
                ctx.stroke();
            }
    ...
    function drawRollerSupport(ctx, x, y, size, color) {
        drawTriangleSupport(ctx, x, y, size, color); // Triangle part
        const rollerRadius = size / 3;
        const rollerYOffset = size / 2.2;
        ctx.beginPath(); ctx.arc(x - size/2 + rollerRadius/2 , y + rollerYOffset, rollerRadius, 0, 2 * Math.PI); ctx.fill();
        ctx.beginPath(); ctx.arc(x + size/2 - rollerRadius/2, y + rollerYOffset, rollerRadius, 0, 2 * Math.PI); ctx.fill();
        // Ground line for roller
        ctx.beginPath();
        ctx.moveTo(x - size * 1.2, y + rollerYOffset + rollerRadius + 3);
    ...
    function drawArrow(ctx, fromx, fromy, tox, toy, color, headlen) {
        ...
        ctx.strokeStyle = color; ctx.fillStyle = color; ctx.lineWidth = 2.5; // Thicker arrow line
    ...
    function drawCrossSectionStress(element_info, results, elementType) {
        ...
        const cs = element_info.cross_section;
        const material = element_info.material;
        ...
        let stressText = `<h4>Stress Distribution (${cs.type || 'N/A'})</h4>`;
        const num_gradient_strips = 60; // Smoother gradient

        csCtx.font = "11px " + themeColors.fontFamily;

        if (cs.type.toLowerCase() === "rectangular") {
            const b_m = (cs.cx_left_m || 0) + (cs.cx_right_m || 0);
            const h_m = (cs.cy_top_m || 0) + (cs.cy_bottom_m || 0);
            if (b_m === 0 || h_m === 0) { stressText += "<p>Invalid section dimensions.</p>"; crossSectionStressInfoDiv.innerHTML = stressText; return; }
            scale = Math.min(availableWidth / b_m, availableHeight / h_m) * 0.85; // More padding
            cs_width_px = b_m * scale;
            cs_height_px = h_m * scale;
            origin_x = canvasWidth / 2;
            origin_y = canvasHeight / 2;

            csCtx.strokeStyle = themeColors.canvasSupportColor; csCtx.lineWidth = 2;
            ...
            cs_width_px = cs_height_px = 2 * r_px; // For NA line drawing
            origin_x = canvasWidth / 2; origin_y = canvasHeight / 2;

            csCtx.strokeStyle = themeColors.canvasSupportColor; csCtx.lineWidth = 2;
            csCtx.beginPath(); csCtx.arc(origin_x, origin_y, r_px, 0, 2 * Math.PI); csCtx.stroke();
        } else {
            stressText += "<p>Cross-section type not drawable yet.</p>"; crossSectionStressInfoDiv.innerHTML = stressText; return;
        }

        csCtx.strokeStyle = themeColors.primary + '99'; csCtx.setLineDash([3, 3]); csCtx.lineWidth = 1.5;
        csCtx.beginPath(); csCtx.moveTo(origin_x - cs_width_px / 2 - 15, origin_y);
        csCtx.lineTo(origin_x + cs_width_px / 2 + 15, origin_y); csCtx.stroke();
        csCtx.fillStyle = themeColors.primary;
        ...
        if (elementType === 'beam') {
            ...
            if (sigma_max_abs_val > 0 && cs.Ix_m4 > 0) {
                ...
                for (let i = 0; i < num_gradient_strips; i++) {
                    ...
                    const color = getColorForStress(sigma_Pa, sigma_max_abs_val || material.Fy_Pa); // Fallback to Fy for scale
            ...
            const axial_stress_Pa = results.axial_stress_Pa || 0; // Assuming +ve P_load from input means compression -> +ve stress_Pa from backend
            ...
            if (Math.abs(axial_stress_Pa) > 0 && material.Fy_Pa > 0) {
                const stress_for_color_viz = -axial_stress_Pa; // getColorForStress expects +ve tension, -ve compression
                ...
                if (cs.type.toLowerCase() === "rectangular") {
                    ...
                    csCtx.strokeStyle = themeColors.canvasSupportColor; csCtx.lineWidth = 2; csCtx.strokeRect(origin_x - cs_width_px / 2, origin_y - cs_height_px / 2, cs_width_px, cs_height_px);
                } else if (cs.type.toLowerCase() === "circular") {
                    csCtx.beginPath(); csCtx.arc(origin_x, origin_y, ((cs.cx_right_m||0) + (cs.cx_left_m||0))/2*scale, 0, 2*Math.PI);
                    csCtx.fill();
                    csCtx.strokeStyle = themeColors.canvasSupportColor; csCtx.lineWidth = 2; csCtx.stroke();
                }
            }
        }
    ...
    function getColorForStress(stress_Pa, max_abs_stress_Pa_scale) {
        if (max_abs_stress_Pa_scale === 0) return 'rgb(230,230,230)';
        const norm_stress = Math.max(-1, Math.min(1, stress_Pa / max_abs_stress_Pa_scale));
        let r, g, b;
        const neutralR = 235, neutralG = 235, neutralB = 235; // Lighter neutral
        const maxIntensity = 255;
        const midIntensityComp = 120; // For non-blue components in compression color
        const midIntensityTens = 120; // For non-red components in tension color

        if (norm_stress > 0) { // Tension - shades of red
            r = maxIntensity;
            g = Math.round(neutralG - (neutralG - midIntensityTens) * norm_stress);
            b = Math.round(neutralB - (neutralB - midIntensityTens) * norm_stress);
        } else if (norm_stress < 0) { // Compression - shades of blue
            const abs_norm_stress = -norm_stress;
            r = Math.round(neutralR - (neutralR - midIntensityComp) * abs_norm_stress);
            g = Math.round(neutralG - (neutralG - midIntensityComp) * abs_norm_stress);
            b = maxIntensity;
        } else {
            r = neutralR; g = neutralG; b = neutralB;
        }
    ...
    function getColorForValue(value, min_val, max_val) { // Viridis-like (for moment gradient)
        let norm_value;
        if (max_val <= min_val) norm_value = 0.0; // Handle edge case or single value
        else norm_value = (value - min_val) / (max_val - min_val);
        norm_value = Math.max(0, Math.min(1, norm_value)); 
        
        // Simple Blue (low) to Red (high) gradient
        const r = Math.round(255 * norm_value);
        const b = Math.round(255 * (1 - norm_value));
        const g = Math.round(Math.min(r,b) * 0.8); // Add some green in the middle for purple-ish transition
        // return `rgb(${r},${g},${b})`;
        // Using a perceptually more uniform colormap is better if available, this is simplified:
        // Viridis (simplified slice for positive values, blue to yellow)
        if (norm_value < 0.1) return "rgb(68,1,84)";  // Deep Purple
        if (norm_value < 0.3) return "rgb(59,82,139)"; // Blue
        if (norm_value < 0.5) return "rgb(33,145,140)";// Teal
        if (norm_value < 0.7) return "rgb(94,201,98)"; // Green
        if (norm_value < 0.9) return "rgb(253,231,37)";// Yellow
        return "rgb(240,128,128)"; // Light Coral for very high values (if scale maxes out)
    }
    ...
    function initializeDisplays() {
        ...
        if(columnEndConditionSelect) columnEndConditionSelect.dispatchEvent(new Event('change')); // This will also trigger K-factor display updates
        else { // Fallback if columnEndConditionSelect is not on page for some reason
             effLengthFactorKxInput.dispatchEvent(new Event('input'));
             effLengthFactorKyInput.dispatchEvent(new Event('input'));
        }
    }
